...
func Test_ErrReadSeekCloser(t *testing.T) {
	...
	t.Run("read error set via option is not overridden", func(t *testing.T) {
		...
		// --- When ---
		have := ErrReadSeekCloser(src, 42, WithReadErr(custom))
...
func Test_ErrorReadSeekCloser_Close(t *testing.T) {
	t.Run("no close error", func(t *testing.T) {
		// --- Given ---
		src := must.Value(os.Open("testdata/file.txt"))
		t.Cleanup(func() { _ = src.Close() })
		rcs := ErrReadSeekCloser(src, -1)
	...
	t.Run("underlying closer error", func(t *testing.T) {
		// --- Given ---
		rcs := ErrReadSeekCloser(sadStruct{}, -1)
	...
	t.Run("custom close error", func(t *testing.T) {
		...
		rcs := ErrReadSeekCloser(sadStruct{}, -1, WithCloseErr(exp))
